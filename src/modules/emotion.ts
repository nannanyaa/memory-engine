...
export async function recordEmotionNode(
  ...
  //   3B 门槛与 ①/② 同款：只有明确高价值情感（highValue）且 confidence>=attachBelow 才写锚点；
  //   否则视为普通对话/技术请求/闲聊，不污染情感锚点池（否则每次对话都灌水、预拉噪音）。
  if (highValue && selected && selected.confidence >= cfg.emotion.attachBelow) {
    ...
    log.debug(
      `[emotion] skip anchor (non-high-value or conf ${selected?.confidence ?? "none"}<${cfg.emotion.attachBelow}): ${o.body.slice(0, 40)}`,
    );
  }
}
